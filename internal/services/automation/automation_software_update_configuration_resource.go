...
func (s *SoftwareUpdateConfigurationModel) ToSDKModel() softwareupdateconfiguration.SoftwareUpdateConfiguration {
	...
	if len(s.Targets) > 0 {
		...
		for _, az := range t.AzureQueries {
			...
			filterOperater := softwareupdateconfiguration.TagOperatorsAll
			tag.FilterOperator = &filterOperater
			if az.TagFilter != "" {
...
func (m SoftwareUpdateConfigurationResource) Create() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, meta sdk.ResourceMetaData) error {
			...
			automationID, _ := automationaccount.ParseAutomationAccountID(model.AutomationAccountID)
			...
			future, err := client.SoftwareUpdateConfigurationsCreate(ctx, id, param, softwareupdateconfiguration.SoftwareUpdateConfigurationsCreateOperationOptions{})
			_ = future
...
func (m SoftwareUpdateConfigurationResource) Read() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, meta sdk.ResourceMetaData) error {
			...
			output.AutomationAccountID = parse.NewAutomationAccountID(id.SubscriptionId, id.ResourceGroupName, id.AutomationAccountName).ID()
