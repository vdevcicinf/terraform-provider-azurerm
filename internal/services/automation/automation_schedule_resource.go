...
func resourceAutomationScheduleCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("start_time"); ok {
		...
		properties.SetStartTimeAsTime(t)
	} else {

		properties.SetStartTimeAsTime(time.Now().Add(time.Duration(7) * time.Minute))
	}

	if v, ok := d.GetOk("expiry_time"); ok {
		t, _ := time.Parse(time.RFC3339, v.(string)) // should be validated by the schema

		expiryTime := date.Time{Time: t}.String()
...
func resourceAutomationScheduleRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if model := resp.Model; model != nil {
		if props := model.Properties; props != nil {
			...
			v, err := props.GetStartTimeAsTime()
			if v != nil && err == nil {

				d.Set("start_time", v.Format(time.RFC3339))
				startTime := v.Format(time.RFC3339)
				print(startTime)
			} /*else if props.StartTime != nil {
				d.Set("start_time", *props.StartTime)
			}*/

			if v, err := props.GetExpiryTimeAsTime(); v != nil && err != nil {
