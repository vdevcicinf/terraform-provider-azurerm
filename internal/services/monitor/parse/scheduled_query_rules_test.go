...
var _ resourceids.Id = ScheduledQueryRulesId{}
...
func TestScheduledQueryRulesIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ScheduledQueryRulesId
	...
	for _, v := range testData {
		...
		actual, err := ScheduledQueryRulesIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
